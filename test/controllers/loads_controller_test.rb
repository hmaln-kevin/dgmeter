...
class LoadsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create load" do
    assert_difference('Load.count') do
      post loads_url, params: { load: { description: @load.description, schedule: @load.schedule, state: @load.state } }
    end
  ...
  test "should update load" do
    patch load_url(@load), params: { load: { description: @load.description, schedule: @load.schedule, state: @load.state } }
