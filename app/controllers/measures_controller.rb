class MeasuresController < ApplicationController
  ...
  def import
    @measure = Measure.new
    @aux = ImportValue.measure
    @aux = @aux[1].lines(',', chomp: true)
    @measure.voltage = @aux[0]
    @measure.current = @aux[1]
    @measure.power = @aux[2]
    @measure.energy = @aux[3]
    @measure.frequency = @aux[4]
    @measure.pf = @aux[5]
    @measure.date = Time.now
    @measure.save

    respond_to do |format|
      format.html { redirect_to measures_url, notice: 'Measure was successfully imported.' }
      format.json { head :no_content }
    end
  ...
  private
    ...
    def measure_params
      params.require(:measure).permit(:date, :voltage, :current, :power, :energy, :frequency, :pf, :device_id)
    end
end
